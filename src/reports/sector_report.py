...
def build_sector_report(sector, companies, ratios):
    safe_name = (
        "".join(c if c.isalnum() or c in (" ", "_", "-") else "_" for c in sector)
        .strip()
        .replace(" ", "_")
    )
    ...
    sector_companies = companies[companies["broad_sector"] == sector].copy()
    ...
    story.append(Paragraph(f"N100 FINANCIAL INTELLIGENCE — {sector}", title_style))
    story.append(
        Paragraph(f"Companies covered: {len(sector_companies)}", styles["Heading2"])
    )
    ...
    for label, column in median_columns:
        if column in sector_ratios.columns:
            value = pd.to_numeric(sector_ratios[column], errors="coerce").median()
        else:
            value = None
    ...
    for _, company in sector_companies.sort_values("company_id").iterrows():
        ...
        row = latest_rows[latest_rows["company_id"].astype(str) == str(ticker)]
...
def main():
    ...
    for sector in sectors:
        try:
            pdf = build_sector_report(sector, companies, ratios)
            generated += 1
    ...
    if errors:
        pd.DataFrame(errors).to_csv(
            PROJECT_ROOT / "output" / "sector_report_errors.csv", index=False
        )
